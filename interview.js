...
function wordSearchLetterCounter(array1, letter) {
  return function () {
    return countTimes(array1, letter);
  };
}
...
function wordSearchLetterCounter(arr, value) {
  let counter = 0;
  for (let i = 0; i < arr.length; i++) {
    counter += countTimes(arr[i], value);
  }
  return counter;
}
...
// inputs: array, callback, and initial value
// outputs: sing value (accumulator)
// reduce array passing callbackFn and initial value
// return result
...
// return? sum of all the prior numbers to to the argument
//function labeled add Numbers

//store our initial value
//num + addNumber(num - 1)
//recursive
//base case if num === 1 then lets return 1
...
// string -> array
// array(reversed)
//for loop run each array
// we will compare the elements in each array.

//if they match up we have palindrome //true after loop checking each element

//if they dont! we dont have palindrome
...
//     if (newAns[i] === false){return false}
//     else{bool}
//   }
// return bool
// }
...
//
//number 50 and less than the number 500
//
//
//
//between50And500
...
// function countChar (str, target, counter = 0, charCounter = 0){
...
// return countChar(str,target,newCounter, charCounter)
// }

//Zack:

// function countChar(input, target, index=0, counter=0){
//   if (input[index] === undefined) return counter;
//   if (input[index] === target){
//      counter += 1;
//   }
//   return countChar(input, target, index+= 1, counter);
// }
...
//input: array and searchVal
//output: number
//function accept array and searchVal as parameters
function countTimes(array, searchVal) {
  //declare variable numOfTime to keep track, set it at 0
  let numOfTimes = 0;
  //iterate through array
  for (let i = 0; i < array.length; i++) {
    //if the element within the array matches the searchVal
    if (array[i] === searchVal) {
      numOfTimes++;
    }
    //increment numOfTimes
  }
  //return numofTimes
  return numOfTimes;
}
...
//function accepts array and letter as its parameter
function wordSearchLetterCounter(array, letter) {
  //find a way to merge subarrays into one array
  let newArr = array.flat();
  //Pass array and letter into countTimes()
  //return countTimes(array, letter)
  return countTimes(newArr, letter);
}
...
//function will accept func as parameter
...
//input: inputStr and targetStr
//output: number

//function accepts two parameter

//declare numOfTimes variable

//iterate over string

//if string index matches targetStr
//increment numOfTimes
...
function memoryMaker() {
  let newArr = [];
  //inner function will accept a value
  return function inner(value) {
    //newArr will need to push values into itself
    newArr.push(value);
    //console.log(newArr)
    //inner function will return an array
    return newArr;
    //this array will consist of all the previously passed values
  };
}
...
//input: array and initial value
//output: number

function sumAllElements(array, initial, sum = 0, i = 0) {
  //base case: exit the function when ?
  if (array[i] === undefined) return sum;
  //add initial value to each element in the array
  sum = initial += array[i];

  //recursive case: return function with original input but increment i by 1
  return sumAllElements(array, initial, sum, i + 1);
}
...
//index are time in minutes
//values are price in dollar
//
function bestProfit(array) {
  //check to see if array is actually an array, if not then return 0;
